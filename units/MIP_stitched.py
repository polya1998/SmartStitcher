...
class MIP_stitched_c(object):
    ...
    def quanzhi_map(self,index,planes_num,result_x,result_y,result_folder,tiles_num,input_folder,pre,mid,end):
        ...
        for i in range(tiles_num):
            ...
            img_path = input_folder + '/' +l_str+'/'+'ds32z8_norm_deskewed_z_'+z_str_i + '.tiff'
                # continue
            # print(img_path)
            # if(l_str[-2:]=='14'):
    ...
    def stitch_one_plane(self,index,planes_num,result_x,result_y,result_folder,tiles_num,input_folder,pre,mid,end,qz_map):
        ...
        z_str = str(index).zfill(5)
        result_plane = np.zeros((result_y, result_x), dtype=np.float16)
        result_path = result_folder + '/MIP_stitched.tiff'
        ...
        for i in range(tiles_num):
            ...
            z_str_i = str(index-temp_t['z']).zfill(5)
            # print(tile_path_list[i])
            img_path = input_folder + '/' +l_str+ '_mip_z.tiff'
                # continue
            print(img_path)
    ...
    def work(self):
        ...
        self.stitch_one_plane(int(self.planes_num/2),self.planes_num,self.result_x,self.result_y,self.result_folder,self.tiles_num,self.input_folder,self.pre,self.mid,self.end,qz_map)
